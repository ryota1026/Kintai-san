...
function doPost(e) {
  var Kintai = init();
  Kintai.receiver.receiveMessage(e.parameters);
}

// Time-based triggerで実行
function confirmSignIn() {
  var Kintai = init();
  Kintai.timesheets.confirmSignIn();
}

// Time-based triggerで実行
function confirmSignOut() {
  var Kintai = init();
  Kintai.timesheets.confirmSignOut();
}
...
function setUp() {
  ...
  if(!global_settings.get('spreadsheet')) {
    ...
    settings.set('無視するユーザ', 'Kintai,GitHub,hubot,slackbot,incoming-webhook');
